...
3. 代理整个主机，而不是只代理一个前缀；必须同时覆盖 `/app`、`/api`、`/raw`、`/clash`、`/clash-ha`、`/surge`、`/loon`、`/quanx`、`/surfboard`、`/singbox` 和 `/smart`。
4. 确认上述转发头正确，并把 Lucky 的精确来源网络加入 `TRUSTED_PROXY_CIDRS`。
5. 为这个虚拟主机关闭 access log，或配置严格脱敏。分享 bearer token 位于 `/raw/<token>`、`/clash/<token>`、`/clash-ha/<token>`、`/surge/<token>`、`/loon/<token>`、`/quanx/<token>`、`/surfboard/<token>`、`/singbox/<token>` 与 `/smart/<token>` 路径，默认访问日志会泄露它。
