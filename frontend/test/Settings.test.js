...
describe('Settings', () => {
  ...
  it('shows all public risks before switching and sends the acknowledgement', async () => {
    ...
    expect(api.request).toHaveBeenNthCalledWith(8, '/api/admin/settings', {
      method: 'PUT',
      body: {
        ...loadedSettings,
        access_mode: 'public',
  ...
  it('keeps passwords empty, verifies confirmation, then clears the session', async () => {
    ...
    expect(api.request).toHaveBeenCalledTimes(7)

    await confirmation.setValue('next-secret')
    await wrapper.get('.credentials-form-grid').trigger('submit')
    await flushPromises()

    expect(api.request).toHaveBeenNthCalledWith(8, '/api/auth/credentials', {
      method: 'PUT',
      body: {
  ...
  it('loads airport credentials without prefilling the password and saves a validated candidate', async () => {
    ...
    expect(api.request).toHaveBeenNthCalledWith(8, '/api/admin/upstream/credentials', {
      method: 'PUT',
  ...
  it('saves a yaml file dropped onto the backup textarea', async () => {
    mockInitialLoad()
    const wrapper = mountSettings()
    await flushPromises()
    const yaml = '# keep\nproxies:\n  - name: bak\n    type: ss\n'
    ...
    await wrapper.vm.onBackupDrop({ dataTransfer: { files: [{ text: async () => yaml }] } })
    await flushPromises()
    expect(api.request).toHaveBeenCalledWith('/api/admin/backup-nodes', {
      method: 'PUT',
      body: { nodes: yaml },
  ...
  it('does not put a large yaml drop into the textarea', async () => {
    mockInitialLoad()
    const wrapper = mountSettings()
    await flushPromises()
    const yaml = `proxies:\n${'  - {name: n, type: ss}\n'.repeat(2000)}`
    ...
    await wrapper.vm.importBackupYaml({
      target: { files: [{ text: async () => yaml }], value: '' },
    })
    await flushPromises()
